                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"TOPOR VPN"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"HiddifyMutex");
  HWND handle = FindWindowA(NULL, "TOPOR VPN");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"TOPOR VPN"))
    {
      return false;
    }
  ...
  if (!window.Create(L"TOPOR VPN", origin, size))
  {
    return EXIT_FAILURE;
  }
